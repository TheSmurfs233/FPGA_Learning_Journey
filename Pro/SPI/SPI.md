...
OLED（OrganicLight-Emitting Diode），又称为有机电激光显示、有机发光半导体（OrganicElectroluminesence Display，OLED）。OLED属于一种电流型的有机发光器件，是通过载流子的注入和复合而致发光的现象，发光强度与注入的电流成正比。OLED在电场的作用下，阳极产生的空穴和阴极产生的电子就会发生移动，分别向空穴传输层和电子传输层注入，迁移到发光层。当二者在发光层相遇时，产生能量激子，从而激发发光分子最终产生可见光。下图是7针OLED模块。

![image-20250120214122850](asset/image-20250120214122850.png)

##### 3.2.2 7针SPI接口OLED显示屏模块引脚描述
...
##### 3.2.3 SSD1306/SSD1315工作原理

屏幕采用SSD1315驱动芯片,下图为不同模式下的引脚分配
![image-20250120220018711](asset/image-20250120220018711.png)
![image-20250120220304616](asset/image-20250120220304616.png)
![image-20250120220405889](asset/image-20250120220405889.png)

在4线SPI模式下多了一个D/C引脚，D/C引脚为低电平时为发命令，为高时为发数据


- SSD1306是一款OLED/PLED点阵 显示屏的控制器，可以嵌入在屏幕中， 用于执行接收数据、显示存储、扫描刷新等任务
